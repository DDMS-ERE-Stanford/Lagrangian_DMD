...
for i = 2:Nt
    y = interp1(Y1(:,i-1),Y2(:,i-1),x,'spline','extrap');
    y = A\y;
    Y2(:,i) = interp1(x,y,Y1(:,i-1),'spline','extrap');
    Y1(:,i) = Y1(:,i-1)+dt*(Y2(:,i)+Y2(:,i-1))/2;
%     x_hat = Y1(:,i-1)+dt/2*Y2(:,i-1);
%     z = interp1(x,0.5*(u(:,i)+u(:,i-1)),x_hat,'linear','extrap');
%     Y1(:,i) = Y1(:,i-1)+dt*z;
% %     Y1(:,i) = Y1(:,i-1)+dt*Y2(:,i-1);
% %     y = interp1(Y1(:,i-1),Y2(:,i-1),x,'spline','extrap');
% %     y = A\y;
% %     Y2(:,i) = interp1(x,y,Y1(:,i),'spline','extrap');
end
toc
tic
X = [Y1;Y2];
...
% y_pod(:,1) =[x;u0];
% y_hat(:,1) = U_k\[x;u0];
% A_hat = U_k'*[diag(ones(Nx,1)),zeros(Nx,Nx);zeros(Nx,Nx),A]*U_k;
% 
% for i = 1:length(t)-1
%     z1 = y_pod(1:Nx,i)+dt/2*y_pod(Nx+1:end,i);
%     z2 = interp1(x,0.5*(u(:,i+1)+u(:,i)),z1,'linear','extrap');
%     y_hat(:,i+1) = A_hat\(y_hat(:,i)+dt*U_k'*[z2;zeros(Nx,1)]);
...
% 
% kappa = 1/10;
% vtmp = ones(length(x)-1,1);
% D2 = (diag(-2*ones(length(x),1))+diag(vtmp,1)+diag(vtmp,-1))/(dx^2);   % second derivative matrix
% D2(1,end) = 1/dx^2; D2(end,1) = 1/dx^2;
% Id = diag(ones(length(x),1));     % identity matrix
% A = Id - dt*(diag(kappa)*D2);      % matrix A  
% 
% %check CFL condition
% CFL = max(abs(u(:,1)))*dt/dx;
...
% print -depsc2 dmd_v_burgers.eps
% 
% figure
% plot(x,u(:,1),'LineWidth',4,'Color',[0.75,0.75,0.75]);
% hold on
...
%     'FontSize',9);
% 
% print -depsc2 pod_v_burgers.eps
% 
% 
% % ref_soln = 0*u_pod;
% % for i =1:Nt
...
%     'FontSize',9);
% 
% print -depsc2 error_v_burgers.eps
% 
...
%     'FontSize',9);
% 
% print -depsc2 error_bd_v_burgers.eps
% 
% 
